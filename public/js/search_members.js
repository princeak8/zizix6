$(document).ready(function() {
    ...
    $('#search').keyup(function(){
      ...
      if(searchVal.length < 3){
        ...
        var ajaxUrl = appUrl+"members_ajax_search";
        $.ajax({
          url:ajaxUrl, 
          ...
          success: function(data) { 
            ...
            var n = 0;
            var acceptRequestUrl = appUrl+"accept_friend_request/";
            var content = '';
            $.each(data, function(key, val) { n++;
              console.log(val.firstname);
              var viewUrl = appUrl+"view_user/"+$.trim(val.firstname)+$.trim(val.lastname);
              var photoUrl = storageUrl+"images/profile_photos/"+val.photo;
